...
class AttnEncoder(nn.Module):
    ...
    def forward(self, driving_x):
        # driving_x: batch_size * time_step * input_size
        batch_size = driving_x.size(0)
        # print(batch_size, self.hidden_size)
        ...
        for t in range(self.T):
            ...
            # batch_size * input_size * time_step
            z1 = self.attn1(x)
...
class AttnDecoder(nn.Module):
    ...
    def __init__(self, input_size, code_hidden_size, hidden_size, time_step):
        ...
        self.tilde = nn.Linear(in_features=self.code_hidden_size + input_size, out_features=input_size)
        self.fc1 = nn.Linear(in_features=code_hidden_size + hidden_size, out_features=hidden_size)
        # 是否要在两个全连接层之间添加激活函数，虽然论文中没有使用
